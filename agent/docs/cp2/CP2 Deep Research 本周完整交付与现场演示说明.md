...
右侧状态栏不是前端自行模拟的步骤，而是读取后端 `ResearchProgress` 返回的 `current_stage`、`stages`、任务计数、Evidence 计数和持久化事件。十二个阶段与实际实现的对应关系如下：

| 右侧显示状态 | 实际处理内容 | 对应实现模块 |
|---|---|---|
| 已创建研究任务 | 创建 Research Job、立即返回 `research_id` 并保存用户请求 | Research Control Plane / Repository |
| 正在生成研究计划 | 根据研究问题和冻结的 SourceManifest 生成动态 Research Plan | Planner |
| 等待确认研究计划 | 暂停执行，等待用户批准或修改计划 | Approval / Plan Revision |
| 研究任务等待执行 | Plan 已批准，等待后台执行器领取 Job | Durable Dispatcher |
| 正在执行研究任务 | 按任务依赖执行本地检索和原文读取，将工具结果转换为 Evidence | Research Graph / Worker / Local Tool Adapter |
| 正在检查资料覆盖度 | 检查 Plan 中各项验收条件是否获得足够 Evidence 支撑 | Coverage Engine |
| 正在整理研究结论 | 将 Finding 和 Evidence 整理为可验证 Claim | Claim Generator |
| 正在检查引用完整性 | 校验 Claim 引用的 Evidence 是否存在、完整且属于冻结 Manifest | Structural Verifier |
| 正在验证研究结论 | 判断 Evidence 是否在语义上真正支持对应 Claim | Semantic Verifier |
| 正在生成研究报告 | 将已验证 Claim、冲突、局限和引用组织为最终 Markdown 报告 | Report Renderer |
| 正在完成研究任务 | 持久化报告、更新 Job 结果状态并发布完成事件 | Graph Finalize / Repository / Event Recorder |
| 研究已完成 | Job 进入 `completed` 终态，报告和历史状态可以查询与恢复 | Research Job Terminal State |

前四个阶段属于 Research Control Plane，用于创建任务、冻结资料范围、制定计划和完成审批；从“正在执行研究任务”到“正在完成研究任务”对应正式 Research Graph 的执行节点；“研究已完成”是 Job 终态，不是额外的研究动作。

#### 2.4.2 进度百分比规则

右侧百分比是面向用户的阶段里程碑，不表示精确剩余时间。当前固定节点为：
...
其中“执行研究任务”阶段会根据 `task_completed / task_total` 在 20%～48% 之间动态计算，其余百分比用于标识当前所处里程碑。阶段状态和起止时间由持久化的 `stage_started`、`stage_completed` 等事件聚合得到，页面刷新后会重新从服务端恢复，而不是依赖浏览器内存。
...
### 3.8 Deep Research 完整架构与流程图（纯文本）

下面的图同时描述用户交互、前后端边界、Research Control Plane、Research Graph、知识库访问、证据链、持久化恢复以及进度展示。箭头表示主要调用或数据流，虚线表示状态、事件和恢复信息。
...
│  │ 对话式 Research 页面                                                 │  │
│  │                                                                       │  │
│  │ 主对话区                                      右侧 Research 状态栏    │  │
│  │ - 研究计划                                    - 当前阶段/总进度       │  │
│  │ - 对话审批与修改                              - Task/Evidence 数量    │  │
│  │ - 执行消息、冲突确认                          - 12 个阶段状态         │  │
│  │ - 最终报告、引用、划词追问                    - 最近事件/刷新恢复     │  │
│  │ - 底部输入框始终保留                                                  │  │
│  └───────────────────────────────┬───────────────────────────────────────┘  │
└──────────────────────────────────┼──────────────────────────────────────────┘
                                   │ HTTP / JSON
...
│                              Web / BFF 层                                   │
│                                                                             │
│  Research API Client                    Research 会话接入                   │
│  - Job / Plan / Progress / Events       - 左侧对话记录                     │
│  - Approve / Revise / Cancel / Retry    - Research 消息持久化              │
│  - Follow-up / Conflict / Report        - 收藏、点赞、点踩                 │
│  - Citation / Evidence                  - 页面路由与刷新恢复               │
└──────────────────────────────────┬──────────────────────────────────────────┘
                                   │ Agent Research API
                                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│                                                                             │
│  创建 Job 并返回 research_id                                               │
│       │                                                                     │
│       ├─► 解析用户指定的 Source Scope                                      │
│       ├─► 冻结 SourceManifest + manifest_hash                              │
│       ├─► Planner 动态生成 Research Plan                                   │
│       ├─► 等待用户批准或修改                                               │
│       ├─► 校验 plan_version + manifest_hash                                │
│       └─► Durable Dispatcher 领取已批准 Job                                │
│                                                                             │
│  created → planning → awaiting_approval → ready                            │
└───────────────┬───────────────────────────────┬─────────────────────────────┘
                │ 冻结资料范围                  │ 已批准任务
                ▼                               ▼
┌──────────────────────────────┐   ┌──────────────────────────────────────────┐
│ 本地知识库 / SourceManifest  │   │          Research Graph Runtime          │
│                              │   │                                          │
│ data-persistence/documents   │   │ START                                    │
│ - doc_id / title             │   │   ▼                                      │
│ - version / updated_at       │   │ prepare：校验已批准上下文                │
│ - authority / rank           │   │   ▼                                      │
│ - content_hash               │◄──┤ execute_tasks                            │
│ - allowed source IDs         │   │   ├─ 按依赖执行 Research Task           │
│                              │   │   ├─ local_search                        │
│ 不联网、不自动扩展资料范围   │   │   ├─ read_document_range                │
└──────────────┬───────────────┘   │   └─ Tool Result → Evidence              │
               │ 原文与元数据      │   ▼                                      │
               └──────────────────►│ coverage：验收条件覆盖度                 │
                                   │   ▼                                      │
                                   │ generate_claims：Finding → Claim         │
                                   │   ▼                                      │
                                   │ structural_verification                  │
                                   │   └─ 验证引用存在、归属和完整性           │
                                   │   ▼                                      │
                                   ...
                                   │   ▼                                      │
                                   │ render_report                            │
                                   │   └─ 生成结论、冲突、局限和 Citation      │
                                   │   ▼                                      │
                                   │ finalize：保存报告并完成 Job             │
                                   │   ▼                                      │
                                   │ END / completed                          │
                                   └───────────────┬──────────────────────────┘
                                                   │
                      ...
                      ▼                            ▼                        ▼
            ┌──────────────────┐        ┌──────────────────┐     ┌──────────────────┐
            │ Evidence 证据链  │        │ 报告与引用       │     │ 事件与 Checkpoint│
            │                  │        │                  │     │                  │
            │ Tool Result      │        │ Verified Claim   │     │ stage_started    │
            │   → Evidence     │        │   → Markdown     │     │ stage_completed  │
            │   → Finding      │        │   → Citation     │     │ task_* / job_*   │
            │   → Claim        │        │   → 原文片段     │     │ report_ready     │
            │   → Verification │        │   → 用户追问     │     │ conflict_resolved│
            └────────┬─────────┘        └────────┬─────────┘     └────────┬─────────┘
                     └───────────────────────────┼────────────────────────┘
                                                 ▼
                                   ┌───────────────────────────┐
                              ...
                              ┌──────────────────┴──────────────────┐
                              ▼                                     ▼
                   页面刷新：重新查询状态                服务中断：读取最后 Checkpoint
                   恢复对话、进度和报告                  从安全、幂等阶段重新执行
```
...
用户
  ├─ 选择“深度研究”并指定本地文件
  ├─ 提交研究问题
  │    └─► 创建 Job ─► 返回 research_id ─► 冻结 SourceManifest
  │                                      └─► 生成动态 Research Plan
  ├─ 查看计划
  │    ├─ 回复修改意见 ─► 新 Plan Version ─► 再次等待审批
  │    └─ 回复“批准” ─► 校验 Plan Version 与 Manifest Hash
  ├─► Dispatcher 启动 Research Graph
  │    └─► Task 执行 ─► 本地检索 ─► 读取上下文 ─► Evidence
  │         └─► Coverage ─► Claim ─► 结构验证 ─► 语义验证 ─► 报告
  ├─ 查看作为助手回复呈现的最终报告
  │    ├─ 点击引用编号 ─► 展开 Evidence 原文片段
  │    ├─ 划词提问 ─► 使用冻结 Evidence 回答
  │    └─ 收藏 / 赞踩 / 下载
  └─ 遇到冲突时通过对话裁决
       ├─ 采用指定来源 / 以最新版为准
       ├─ 保留双方并注明适用范围
       └─ 继续读取上下文后判断
            └─► 保存 conflict_resolved 事件 ─► 更新报告结论
```

#### 3.8.2 状态与恢复旁路

```text
每个 Control Plane / Graph 阶段
        ├─ 更新 ResearchJob.current_stage
        ├─ 保存当前阶段产物
        ├─ 写入 stage_started / stage_completed / task_* / job_* 事件
        └─ 保存 WorkflowCheckpoint
                    │
                    ├─► Progress Service 聚合
                    │      └─► 右侧状态栏展示阶段、百分比、任务和证据数量
                    ├─► 页面刷新
                    │      └─► 根据 research_id 恢复 Job、Plan、Events 和 Report
                    └─► 服务中断后重试
                           └─► 校验 Plan Version 和安全阶段
                                  └─► 从最后 Checkpoint 幂等恢复
```
