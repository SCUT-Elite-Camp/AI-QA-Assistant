...
class Clarifier:
    ...
    def _system_prompt() -> str:
        return (
            ...
            "先向用户澄清。核心原则：默认优先进行知识库检索，绝不轻易发起澄清。"
            "只要问题涉及项目、架构、技术名词（如 Agent、RAG、API 等），"
            "或者包含'检索'、'搜索'、'回答'、'知识库'等指令，必须设 needs_clarification 为 false，"
            "直接允许系统检索知识库。仅在用户仅输入无意义单字或彻底无法生成检索词时才要求澄清。"
            "只返回 JSON，不要使用 Markdown。格式为："
            '{"needs_clarification":false,"question":"","reason":"..."}'
        )
