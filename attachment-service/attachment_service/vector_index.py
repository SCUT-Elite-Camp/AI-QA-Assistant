...
class AttachmentSectionVectorIndex(AttachmentVectorIndex):
    """Independent Milvus lifecycle for Section navigation vectors."""

    def __init__(self) -> None:
        super().__init__()
        self.enabled = os.getenv(
            "ATTACHMENT_SECTION_VECTOR_INDEX_ENABLED", "false"
        ).lower() in {"1", "true", "yes"}
        self.collection = os.getenv(
            "ATTACHMENT_SECTION_MILVUS_COLLECTION", "attachment_sections_bgem3"
        )
        forbidden = {
            os.getenv("MILVUS_COLLECTION", "doc_chunks_cp2_bgem3").strip().casefold(),
            os.getenv("ATTACHMENT_MILVUS_COLLECTION", "attachment_evidence_cp2").strip().casefold(),
        }
        if self.enabled and self.collection.strip().casefold() in forbidden:
            raise RuntimeError("attachment_section_collection_must_be_isolated")
    ...
    def replace(self, attachment_id: str, items: list[dict[str, Any]]) -> None:
        if os.name == "nt" and os.getenv("ATTACHMENT_VECTOR_CHILD") != "1":
            ...
            completed = subprocess.run(
                [sys.executable, "-m", "attachment_service.vector_worker"],
                input=json.dumps({
                    "kind": "section", "attachment_id": attachment_id, "items": items,
                }, ensure_ascii=False),
                text=True,
                cwd=str(Path(__file__).resolve().parents[1]),
                env=environment,
            ...
            if completed.returncode != 0:
                raise RuntimeError("attachment_section_vector_index_unavailable")
            return
        self._replace_in_process(attachment_id, items)
    ...
    def _replace_in_process(self, attachment_id: str, items: list[dict[str, Any]]) -> None:
        ...
        store.collection.delete(expr=f'doc_id == "{attachment_id}"')
        store.collection.flush()
        rows = [item for item in items if str(item.get("navigation_text") or "").strip()]
        if not rows:
            return
        texts = [str(item["navigation_text"]) for item in rows]
        embeddings = embed_texts(texts)
        store.insert_chunks(
            embeddings=embeddings,
            chunk_ids=[str(item["id"]) for item in rows],
            chunk_texts=texts,
            doc_ids=[attachment_id] * len(rows),
            chunk_indices=list(range(len(rows))),
            source_urls=[""] * len(rows),
            titles=[str(item.get("title") or "") for item in rows],
            spaces=["section"] * len(rows),
            doc_types=["section"] * len(rows),
            collection_name=self.collection,
        )
    ...
    def search_by_vector(
        ...
        *, evidence_ids: list[str] | None = None,
        ...
        return [{
            ...
            "navigation_text": hit.entity.get("chunk_text"),
