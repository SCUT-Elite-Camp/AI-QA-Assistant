...
// 文件可见范围的中文描述（仅展示已配置的授权信息）
const scopeLabel = (item: FileItem): string => {
  if (item.visibility === 'shared') return '全员'
  const grants = item.grants ?? []
  if (grants.length === 0) return '仅自己'
  if (grants.some(g => g.grantType === 'public')) return '全员'
  if (grants.some(g => g.grantType === 'department')) return '部分部门'
  if (grants.some(g => g.grantType === 'user')) return '部分用户'
  return '仅自己'
}
