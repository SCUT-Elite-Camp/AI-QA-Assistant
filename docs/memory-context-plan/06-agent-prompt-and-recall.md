...
前置：`04a`、`05`、`06a`、`06b`。负责人：指定 Agent 集成人。后续依赖：`07`、`09-Agent`。
...
4. Agent 的私有 `/api/internal/chat` 返回 `InternalChatResponse`；其中 `response` 是原有 ChatResponse，`memory_decision.fact_proposals` 仅由 BFF 消费。`01` 至 `08` 阶段该数组必须固定为空，Fact proposal/confirm/revoke 只能由 `09a-Web`、`09-Agent`、`09-Web` 启用。压缩计划不在此时返回，必须等助手消息成功落库后由 `07` 的专用端点生成。不能把 Fact metadata 暴露给普通浏览器响应。
