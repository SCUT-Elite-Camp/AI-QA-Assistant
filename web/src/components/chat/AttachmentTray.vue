...
function openPicker() {
  if (!props.disabled && serviceEnabled.value) inputRef.value?.click()
}
...
<template>
  <div class="w-full" @dragover.prevent @drop.prevent="onDrop">
    <UButton icon="i-lucide-paperclip" color="neutral" variant="ghost" size="sm" :disabled="disabled || !serviceEnabled" :title="serviceEnabled ? '上传图片或文件' : '附件服务不可用'" aria-label="上传图片或文件" @click="openPicker" />
