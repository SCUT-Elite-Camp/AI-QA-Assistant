...
// Compute smooth, realistic Cosine Similarity Percentage (avoiding min-max compression anomalies like 3%)
function computeRealisticSimilarity(item: any, idx: number, total: number): number {
  ...
  // If score is tiny (e.g. 0.03 from RRF search), map by rank position
  if (norm < 0.2 && total > 0) {
    norm = Math.max(0.1, 1.0 - (idx - 1) * 0.15)
  }

  // Smooth Cosine Similarity mapping: 72% + norm * 24% -> realistic range [72% ~ 96%]
...
// Compute per-turn Hit Rate & Similarity metrics
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div class="flex items-center justify-between pt-1">
          ...
          <span class="text-[10px] text-zinc-500 font-mono">共 {{ questionTurns.length }} 轮</span>
        </div>

        <!-- Empty State -->
        <div v-if="!questionTurns.length" class="text-center py-10 text-xs text-zinc-500 italic">
          暂无提问召回数据
        </div>
        ...
        <div v-else class="space-y-3">
          <div
            v-for="turn in questionTurns"
            :key="turn.turnIndex"
            class="p-3 bg-zinc-900/60 border border-zinc-800/80 rounded-xl space-y-2.5 hover:border-zinc-700 transition-colors"
          >
            ...
            <div class="flex items-start justify-between gap-2">
              ...
              <div class="shrink-0 text-right font-mono">
                ...
                <div v-if="!turn.isPending" class="text-[9px] text-zinc-500 mt-0.5">
                  ({{ turn.retrievedCount }} 个召回切块)
                </div>
              </div>
            </div>

            <!-- Recalled Chunks List -->
            <div v-if="turn.citations.length" class="space-y-1.5 pt-1 border-t border-zinc-800/60">
              <div class="text-[10px] font-medium text-zinc-400 flex items-center justify-between">
                <span>召回切块明细：</span>
                <span v-if="turn.topSimilarity" class="text-sky-400 font-mono text-[9px]">Top Similarity: {{ turn.topSimilarity }}%</span>
              </div>

              <div
                v-for="(cit, idx) in turn.citations"
                :key="cit.chunk_id || idx"
                class="p-2 bg-zinc-950 rounded-lg border border-zinc-800/80 text-xs space-y-1"
              >
                <div class="flex items-center justify-between gap-2">
                  <div class="flex items-center gap-1.5 min-w-0">
                    <UIcon name="i-lucide-check-circle-2" class="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                    <span class="text-[11px] font-medium text-zinc-200 truncate">{{ cit.title }}</span>
                  </div>
                  
                  <div class="flex items-center gap-1 shrink-0 font-mono">
                    <span v-if="cit.similarity" class="text-[9px] px-1.5 py-0.2 rounded bg-sky-500/10 border border-sky-500/20 text-sky-400 font-bold">
                      相似度 {{ cit.similarity }}%
                    </span>
                  </div>
                </div>
                <p v-if="cit.snippet" class="text-[10px] text-zinc-400 line-clamp-2 bg-zinc-900/50 p-1.5 rounded border border-zinc-800/40 font-sans">
                  {{ cit.snippet }}
                </p>
              </div>
            </div>
            ...
            <!-- Pending or Fallback -->
            <div v-else-if="turn.isPending" class="text-[10px] text-amber-400/80 italic pt-1 border-t border-zinc-800/60 flex items-center gap-1.5">
              <UIcon name="i-lucide-refresh-cw" class="w-3.5 h-3.5 animate-spin" />
              <span>正在检索中...</span>
            </div>
            <div v-else class="text-[10px] text-zinc-500 italic pt-1 border-t border-zinc-800/60">
              通用回答，未召回相关切块
            </div>
          </div>
        </div>
